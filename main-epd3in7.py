...
def main(argv):
    global DIST, RAINFALL_DIST, VERBOSE_FLAG, ROTATE_FLAG, IMAGEOUT_FLAG, ROTATE_DEGREE, DRYRUN_FLAG

    opts, args = getopt.getopt(argv[1:],'d:r:vhiR:',["district=","rainfall-district=","verbose","image-save","help","rotate-display=","dry-run"])
    for opt,arg in opts:
    ...
    try:
        ...
        if (ROTATE_FLAG): 
            ...
            frame = frame.rotate(ROTATE_DEGREE)
        
        if (VERBOSE_FLAG): print("[INFO] Drawing to the e-paper display...")
        if not (DRYRUN_FLAG): epd.display_4Gray(epd.getbuffer_4Gray(frame))    
